...
- (UILabel *)txtLabel
{
    if (!_txtLabel) {
        ...
        _txtLabel.font = [UIFont htmi_systemFontOfSize:15];
    }
    return _txtLabel;
}
...
- (UIButton *)button
{
    if (!_button) {
        ...
        _button.titleLabel.font = [UIFont htmi_systemFontOfSize:17];
...
- (HTMINetTipView *)getNetTipView
{
    UIView *view = [self getViewWithTag:kNetTipViewTag];
    
    if (view && [view isKindOfClass:[HTMINetTipView class]]){
        
        return (HTMINetTipView *)view;
    }
    else
    {
        return nil;
...
{
    HTMINetTipView *view = [self getNetTipView];
    
    if (!view) {
        view = [[HTMINetTipView alloc] initWithFrame:self.bounds];
