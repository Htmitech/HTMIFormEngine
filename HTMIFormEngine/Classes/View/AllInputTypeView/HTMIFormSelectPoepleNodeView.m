//
//  HTMIFormSelectPoepleNodeView.m
//  MXClient
//
...
//#import "HTMISYS_DepartmentModel.h"
...
@implementation HTMIFormSelectPoepleNodeView
...
- (void)inputTypeFieldItem:(HTMIFieldItemModel *)fieldItem name:(NSString *)nameString value:(NSString *)valueString width:(CGFloat)itemWidth totalView:(UIView *)view cellHeight:(CGFloat)cellHeight {
    ...
    if ((![fieldItem.modeString isEqual:[NSNull null]] && [fieldItem.modeString isEqualToString:@"1"])) {
        ...
        if (self.tabFormModel.tabStyle == 0) {
            ...
            [self netWorkFormSelectPeopleOrNode:view];
        }
...
- (UIView *)creatBorderView:(UIView *)borderView borderRect:(CGRect)borderRect {
    ...
    if (self.fieldItemModel.valueString.length < 1) {
        valueLabel.text = FormGetHTMILocalString(@"htmi_common_pleasechoosedepeoplepartment");
...
- (void)selecetNodeAndPeople:(UITapGestureRecognizer *)tap {
    ...
    if ([self.delegate respondsToSelector:@selector(selectNodeOrPeopleType:fieldItem:baseView:)]) {
        [self.delegate selectNodeOrPeopleType:6//ChooseTypeOrganization
                                    fieldItem:fieldItem
                                     baseView:self];
    }
...
#pragma mark ------ 互联网表单
- (void)netWorkFormSelectPeopleOrNode:(UIView *)totalView {
