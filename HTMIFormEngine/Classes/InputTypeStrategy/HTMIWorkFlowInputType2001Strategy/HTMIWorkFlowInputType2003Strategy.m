//
//  HTMIWorkFlowInputType2003Strategy.m
//  MXClient
//
...
@implementation HTMIWorkFlowInputType2003Strategy
    ...
    if ((![self.fieldItemModel.modeString isEqual:[NSNull null]] && [self.fieldItemModel.modeString isEqualToString:@"1"])) {
        if (self.fieldItemModel.nameVisible) {
            if (self.fieldItemModel.nameRN) {//显示name并且折行
                ...
                CGFloat editH = [NSString labelSizeWithMaxWidth:self.fieldItemModel.finalItemWidth-stringLeftWidth*2 content:self.fieldItemModel.valueString FontOfSize:self.fieldItemModel.formLabelFont].height+stringTopHeight*2;
                
                CGFloat height = self.fieldItemModel.valueString.length>0 ? MAX(editH, kH6(50)) : 0;
