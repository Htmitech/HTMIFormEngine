...
@interface ACSelectMediaView_form ()<UICollectionViewDelegate, UICollectionViewDataSource, TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MWPhotoBrowserDelegate>
...
@property (nonatomic, copy) ACMediaDeleteBlock deleteBlock;
...
@implementation ACSelectMediaView_form
...
- (void)configureCollectionView {
    ...
    [_collectionView registerClass:[ACMediaImageCell_form class] forCellWithReuseIdentifier:NSStringFromClass([ACMediaImageCell_form class])];
...
- (UIViewController *)rootViewController {
    if (!_rootViewController) {
        ...
        _rootViewController = [CurrentViewController currentViewController];
    }
...
- (void)setPreShowMedias:(NSArray *)preShowMedias {
    ...
    for (id object in preShowMedias) {
        ACMediaModel_form *model = [ACMediaModel_form new];
        ...
        }else if ([object isKindOfClass:[ACMediaModel_form class]]) {
            model = object;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ACMediaImageCell_form *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ACMediaImageCell_form class]) forIndexPath:indexPath];
    if (indexPath.row == _mediaArray.count) {
        ...
        ACMediaModel_form *model = _mediaArray[indexPath.row];
        ...
        [cell setACMediaClickDeleteButton:^{
            ACMediaModel_form *model = _mediaArray[indexPath.row];
            //wlq update 增加删除回调
            !_deleteBlock ?  : _deleteBlock(model);
            
            if (!_allowMultipleSelection) {
                
                for (NSInteger idx =0; idx < _selectedImageModels.count; idx++) {
                    if ([_selectedImageModels[idx] isEqual:model]) {
                        
                        [_selectedImageAssets removeObjectAtIndex:idx];
                        [_selectedImageModels removeObjectAtIndex:idx];
                        break;
                    }
                }
                
                for (ACMediaModel_form *object in _selectedVideoModels) {
                    if ([object isEqual:model]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _mediaArray.count) {
        switch (_type) {
            ...
            case ACMediaTypeVideo:
                [self openVideo];
                break;
            case ACMediaTypeVideoAndTape:
            {
                ACAlertController *alert = [[ACAlertController alloc] initWithActionSheetTitles:@[@"视频", @"录像"] cancelTitle:@"取消"];
                [alert clickActionButton:^(NSInteger index) {
                    if (index == 0) {
                        [weakSelf openVideo];
                    }else {
    ...
    //展示媒体
        ...
        for (ACMediaModel_form *model in _mediaArray) {
...
- (void)layoutCollection {
    ...
    _collectionView.height = allImageCount == 0 ? 0 : maxRow * self.frame.size.width/4;
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSInteger index = 0; index < assets.count; index++) {
            ...
            [[ACMediaManager_form manager] getMediaInfoFromAsset:asset completion:^(NSString *name, id pathData) {
                ACMediaModel_form *model = [[ACMediaModel_form alloc] init];
                model.name = name;
                ...
                if (!_allowMultipleSelection) {
                    //用数组是否包含来判断是不成功的。。。
                    for (ACMediaModel_form *md in _selectedImageModels) {
                ...
                if (index == assets.count - 1) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (!_allowMultipleSelection) {
                            ...
                            for (ACMediaModel_form *selectModel in _selectedImageModels) {
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingVideo:(UIImage *)coverImage sourceAssets:(id)asset {
    ...
    [[ACMediaManager_form manager] getMediaInfoFromAsset:asset completion:^(NSString *name, id pathData) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!_allowMultipleSelection) {
                //用数组是否包含来判断是不成功的。。。
                for (ACMediaModel_form *tmp in _selectedVideoModels) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([mediaType isEqualToString:@"public.movie"]) {
        ...
        [[ACMediaManager_form manager] getVideoPathFromURL:videoAssetURL PHAsset:asset enableSave:NO completion:^(NSString *name, UIImage *screenshot, id pathData) {
            ACMediaModel_form *model = [[ACMediaModel_form alloc] init];
        ...
        [[ACMediaManager_form manager] getImageInfoFromImage:image PHAsset:asset completion:^(NSString *name, NSData *data) {
            ACMediaModel_form *model = [[ACMediaModel_form alloc] init];
            model.image = image;
            model.name = name;
